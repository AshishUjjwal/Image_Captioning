...
def load_vocab(file_path):
    ...
    try:
        vocab = np.load(file_path, allow_pickle=True).item()
    except Exception as e:
        raise ValueError(f"Error loading {file_path}: {e}")
    
    return vocab

# Adjust the path to your vocab1.npy file
file_path = 'vocab.npy'
...
# Verify loaded vocab
print(f"Vocab loaded. Number of items: {len(vocab)}")
...
# Call models to get outputs #############################################
image_model_output = image_model(image_input)
language_model_output = language_model(language_input)
